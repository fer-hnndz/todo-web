...
def index():
    ...
    if add_task_form.validate_on_submit():
        ...
        try:
            print(
                "Date y Time del form",
                add_task_form.due_date.data,
                add_task_form.due_time.data,
            )

            print(
                "Types",
                type(add_task_form.due_date.data),
                type(add_task_form.due_time.data),
            )

            due = datetime.combine(
                add_task_form.due_date.data, add_task_form.due_time.data
            )

            print("Due date and time:", due, type(due))
            if due < datetime.now():
                flash("Due date and time cannot be in the past.", "danger")
                return redirect(url_for("index"))

            print("Creating task object....")
            task = Task(
                name=add_task_form.task_name.data,
                owner_id=current_user.id,
                due=due,
            )
            print("Task object created:", task)
    ...
    tasks = (
        Task.query.filter_by(owner_id=current_user.id).order_by(Task.id.desc()).all()
    )
